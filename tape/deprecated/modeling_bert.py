...
class ProteinBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.megatron = config.megatron
        if not config.megatron:
...
class ProteinBertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        block1_after_dense = self.dense(hidden_states)
        block1_after_dense_drop = self.dropout(block1_after_dense)
        post_attn_layernormed = self.LayerNorm(block1_after_dense_drop + input_tensor)
        residual1 = block1_after_dense_drop + input_tensor

        return post_attn_layernormed, residual1
...
class ProteinBertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, unnormed_input):
        self_outputs = self.self(input_tensor, attention_mask)
        attention_output, block_neck = self.output(self_outputs[0], unnormed_input)
        outputs = (attention_output,) + self_outputs[1:]  # add attentions if we output them
        return outputs, block_neck


# Corresponding to MLP in Megatron
...
class ProteinBertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.megatron = config.megatron
        if self.megatron:
            self.InputLayerNorm = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        ...
        if self.megatron:
            self.TailDense = nn.Linear(config.intermediate_size, config.hidden_size)
    ...
    def forward(self, hidden_states, attention_mask):
        if not self.megatron:
            ...
            normed_hidden_states = self.InputLayerNorm(hidden_states)

            attention_outputs, neck = self.attention(normed_hidden_states, attention_mask, hidden_states)

            attention_output = attention_outputs[0]

            intermediate_output = self.intermediate(attention_output)

            tail_dense = self.TailDense(intermediate_output)

            layer_output = tail_dense + neck
...
class ProteinBertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        if not self.megatron:
            ...
            # take the first hidden state (as pooling)
            # first_token_tensor = hidden_states[:, 0]
            # return self.LayerNorm(first_token_tensor)

            pooled_out = self.LayerNorm(hidden_states)
            first_token_tensor = pooled_out[:, 0]
            densed_pooled_out = self.dense(first_token_tensor)

            return self.activation(densed_pooled_out)
